...
function FormInputField(props: Props) {
	...
	return (
		<div className={clsx(mainClassName, className)}>
			<label htmlFor={inputProps.name} className={clsx('input-field-label', joinClsx(className, 'input'))}>
				{label}
			</label>
			<input className={clsx('input-field')} {...inputProps} />
			{isError && <div className={clsx('input-field-error-feedback', joinClsx(className, 'error-feedback'))}>{error}</div>}
		</div>
	);
}
